    ...
    render()
    {
        ...
        return (
            <>
                ...
                { value === 'users' && (
                    <div className='col bookshelf-search-user-results'>
                        { userSearch.map(({ username, profile_picture }) => (
                            <div key={ username } className='row align-center bookshelf-search-user-result' onClick={ () => Router.push(`${ base }user?name=${ username }`) }>
                ...
                { value === 'books' && (
                    <div className='col bookshelf-search-book-results'>
                        { bookSearch.map(result => (
                            <div key={ result.book_id } className='row align-center bookshelf-search-book-result'>
                                { result.volumeInfo.imageLinks && result.volumeInfo.imageLinks.smallThumbnail && (
                                    <img src={ result.volumeInfo.imageLinks.smallThumbnail } className='bookshelf-search-book-result-image' /> )}
                                { (!(result.volumeInfo.imageLinks) || !(result.volumeInfo.imageLinks.smallThumbnail)) && (
                                    <img src='https://books.google.com/googlebooks/images/no_cover_thumb.gif' className='bookshelf-search-book-result-image' /> )}
                                <div className='col bookshelf-search-book-result-info'>
                                    ...
                                    <Typography variant='body1' color='default' className='black center'>
