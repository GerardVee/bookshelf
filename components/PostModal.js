...
const styles = () => ({ maxHeight: { maxHeight: 250 } });
...
export default withStyles(styles)(connect(mapStateToProps, mapDispatchToProps)(class extends Component
{
    ...
    render()
    {
        const { open, close, user, classes } = this.props;
            ...
            >
                ...
                <DialogContent>
                    ...
                    <div className='row justify-space-between align-center'>
                        <Button onClick={ (e) => this.setState({ anchorEl: e.currentTarget }) }
                            variant='text' color={ aboutEmpty ? 'primary' : 'default' }
                    ...
                    <Menu id='long-menu' anchorEl={ anchorEl } open={ menuIsOpen } onClose={ () => this.setState({ anchorEl: null }) }
                        PaperProps={{ className: classes.maxHeight }}
                    >
                        <MenuItem onClick={ () => this.selectItem('No Book', 'NONE', 'NONE') }>No Book</MenuItem>
                        { books.reading && books.reading.title && (
                            <MenuItem onClick={ () => this.selectItem(books.reading.title, 'READING', books.reading.book_id) }>
                                { trunc(books.reading.title) }
                            </MenuItem> )}
                        { books.have_read.map(book => (
                            <MenuItem key={ book.book_id } onClick={ () => this.selectItem(book.title, 'READ', book.book_id) }>
                                { trunc(book.title) }
                            </MenuItem> ))}
                        { books.will_read.map(book => (
                            <MenuItem key={ book.book_id } onClick={ () => this.selectItem(book.title, 'WILL_READ', book.book_id) }>
                                { trunc(book.title) }
                            </MenuItem> ))}
                    </Menu>
                </DialogContent>
