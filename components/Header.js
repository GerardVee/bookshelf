...
import { getAllNotifications, receiveNotification } from '../ducks/actions';
...
export default connect(mapStateToProps, mapDispatchToProps)(class extends Component
{
    ...
    configureCredentials()
    {
        ...
        AWS.config.credentials = new AWS.CognitoIdentityCredentials({ IdentityPoolId: process.env.IDENTITY_POOL_ID });
        const cognitoIdentity = new AWS.CognitoIdentity();
        AWS.config.credentials.get((err) =>
        {
            if (!err)
            {
                const params = { IdentityId: AWS.config.credentials.identityId };
                cognitoIdentity.getCredentialsForIdentity(params, (_, data) =>
                {
        ...
        return (
            <AppBar position='sticky'>
                <Toolbar className='bookshelf-toolbar' variant='dense'>
                    <Typography variant='headline' color='default' className='bookshelf-toolbar-title'
                        ...
                        <div className='row'>
                            ...
                            >
                                { (user.notifications.length > 0 && (user.notifications.filter(n => !n.read).length > 0)) && (
                                    <Badge badgeContent={ user.notifications.filter(n => !n.read).length } color='secondary'>
                                        <Notifications />
                                    </Badge> )}
                                { (user.notifications.length === 0 || (user.notifications.filter(n => !n.read).length === 0)) && (
                                    <Notifications /> )}
