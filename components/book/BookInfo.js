import { removeReadBook, removeWillReadBook, selectReadBook, selectReadingBook, selectWillReadBook } from '../../ducks/actions';
...
export default connect(mapStateToProps, mapDispatchToProps)(({
    myUser, id, title, authors, publishedDate,
    description, averageRating, imageLinks, industryIdentifiers,
    selectReadingBook, selectReadBook, removeReadBook, selectWillReadBook, removeWillReadBook
}) =>
{
    ...
    return (
        <div className='col bookshelf-book-main'>
                ...
                <div className='col bookshelf-book-main-stats'>
                    ...
                    <div className='row'>
                        <NoSsr>
                            ...
                            { !(currentlyReading) && !(currentlyReadingNone) && (
                                <Tooltip title='This will replace your current book'>
                                    <Button variant='contained' color='primary'
                                        onClick={ () =>{ selectReadBook(myUser, myUser.books.reading); selectReadingBook(myUser, thisBook); } }
                                    >
                                        Reading
                                    </Button>
                                </Tooltip> )}
                            { currentlyReadingNone && (
                                <Button variant='contained' color='primary' onClick={ () => selectReadingBook(myUser, thisBook) }>Reading</Button> )}
                            { currentlyReading && <Button variant='contained' color='default' disabled>Read</Button> }
                            { hasRead && (
                                <Button variant='contained' color='secondary'
                                    onClick={ () => removeReadBook(myUser, myUser.books.have_read.findIndex(bk => bk.book_id === id)) }
                                >
                                    Remove Read
                                </Button> )}
                            { !(currentlyReading) && !(hasRead) && (
                                <Button variant='contained' color='default'
                                    onClick={ () => selectReadBook(myUser, thisBook) }
                                >
                                    Read
                                </Button> )}
                            ...
                            { willHaveRead && (
                                <Button variant='contained' color='secondary'
                                    onClick={ () => removeWillReadBook(myUser, myUser.books.will_read.findIndex(bk => bk.book_id === id)) }
                                >
                                    Remove Will Read
                                </Button> )}
                            { !(currentlyReading) && !(willHaveRead) && (
                                <Button variant='contained' color='default'
                                    onClick={ () => selectWillReadBook(myUser, thisBook) }
                                >
                                    Will Read
                                </Button> )}
                        </NoSsr>
            ...
            { industryIdentifiers && (
                <Typography variant='body2' color='default' className='center bookshelf-book-main-stats-isbn'>
                    ISBN10 { industryIdentifiers.find(({ type }) => type === 'ISBN_10').identifier || 'N/A' }
                </Typography> )}
            { industryIdentifiers && (
                <Typography variant='body2' color='default' className='center'>
                    ISBN13 { industryIdentifiers.find(({ type }) => type === 'ISBN_13').identifier || 'N/A' }
                </Typography> )}
            <Typography variant='subheading' color='default' className='bookshelf-book-main-description'>
